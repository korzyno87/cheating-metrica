...
class MAIN:
    ...
    def get_chromedriver(self, proxy=None, agent=None):
        ...
        if self.SETTINGS.get("ROLL_UP") == "True":
    ...
    def new_proxy_list(self):
        if self.SETTINGS.get("DEFAULT_PROXY") == "random":
            ...
            if choice == "proxybroker2":
                proxys_list = self.PROXYS.get_proxys_list_proxybroker2(
                    limit=self.SETTINGS.get("LENGHT_PROXYS_LIST")
                )
            if choice == "FreeProxy":
                proxys_list = self.PROXYS.get_proxys_list_FreeProxy(
                    limit=self.SETTINGS.get("LENGHT_PROXYS_LIST")
                )
